...
class sphy(pcrm.DynamicModel):
	def __init__(self, cfgfile):
		...
		self.LakeFLAG = self.config.getint('MODULES','LakeFLAG')
		...
		syF = self.config.getint('TIMING', 'startyear_F')
		smF = self.config.getint('TIMING', 'startmonth_F')
		sdF = self.config.getint('TIMING', 'startday_F')
		...
		if self.GlacFLAG:
			...
			self.GlacTable = pd.read_csv(os.path.join(self.inpath, self.config.get('GLACIER', 'GlacTable')))
			...
			self.ModelID = pcr.pcr2numpy(pcr.readmap(os.path.join(self.inpath, self.config.get('GLACIER', 'ModelID'))), self.MV) #-Nominal
			...
			self.TLapse_table = pd.read_csv(self.inpath + self.config.get('CLIMATE','TLapse'), header=None, index_col=0, sep=' ', skipinitialspace =True)
			#-Map with glacier IDs
			self.GlacID = pcr.readmap(self.inpath + self.config.get('GLACIER','GlacID'))
			#-Check if glacier retreat should be calculated
			self.GlacRetreat = self.config.getint('GLACIER','GlacRetreat')
			if self.GlacRetreat == 1:
				#-Get date for updating the glacier fraction (once a year)
				GlacUpdate = self.config.get('GLACIER', 'GlacUpdate').split(',')
				self.GlacUpdate = {}
		...
		if self.LakeFLAG == 1:
			...
			self.LakeID = pcr.cover(pcr.readmap(self.inpath + self.config.get('LAKE','LakeId')), 0)
			# lookup table with function for each lake (exp, 1-order poly, 2-order poly, 3-order poly)
			LakeFunc_Tab = self.inpath + self.config.get('LAKE', 'LakeFunc')
			# lookup table with Qh-coeficients for each lake
			LakeQH_Tab = self.inpath + self.config.get('LAKE', 'LakeQH')
			# lookup table with Sh-coeficients for each lake
			LakeSH_Tab = self.inpath + self.config.get('LAKE', 'LakeSH')
			# lookup table with hS-coeficients for each lake
			LakeHS_Tab = self.inpath + self.config.get('LAKE', 'LakeHS')
			...
			try:
				self.UpdateLakeLevel = pcr.readmap(self.inpath + self.config.get('LAKE','updatelakelevel'))
				self.LLevel = self.inpath + self.config.get('LAKE','LakeFile')
		...
		if self.ResFLAG == 1:
			...
			self.ResID = pcr.cover(pcr.readmap(self.inpath + self.config.get('RESERVOIR','ResId')), 0)
			# lookup table with operational scheme to use (simple or advanced)
			ResFunc_Tab = self.inpath + self.config.get('RESERVOIR', 'ResFuncStor')
			# Reservoir function
			self.ResFunc = pcr.cover(pcr.lookupscalar(ResFunc_Tab, 1, self.ResID), 0)
			try:
				# lookup table with coefficients for simple reservoirs
				ResSimple_Tab = self.inpath + self.config.get('RESERVOIR', 'ResSimple')
			...
			try:
				...
				ResAdvanced_Tab = self.inpath + self.config.get('RESERVOIR', 'ResAdv')
	...
	def initial(self):
		...
		if self.LakeFLAG == 1 or self.ResFLAG == 1:
			...
			if self.LakeFLAG == 1:
				LakeStor_Tab = self.inpath + self.config.get('LAKE', 'LakeStor')
			...
			if self.ResFLAG == 1:
				ResStor_Tab = self.inpath + self.config.get('RESERVOIR', 'ResFuncStor')
		...
		if self.GlacFLAG == 1:
			...
			self.GlacID_flag = self.config.getint('REPORTING', 'GlacID_flag')
			if self.GlacID_flag:
				self.GlacVars = self.config.get('REPORTING', 'GlacID_report').split(',')  #-get the variables to report
				self.glacid = sorted(self.GlacTable['GLAC_ID'].unique())  #-get the unique glacier ids
				self.GlacID_memerror = self.config.getint('REPORTING', 'GlacID_memerror') #-check if a memory error occured while runnning the model
		...
		if self.LakeFLAG == 1 and self.config.getint('REPORTING', 'Lake_wbal') ==1:
		...
		if self.ResFLAG == 1 and self.config.getint('REPORTING', 'Res_wbal') == 1:
