...
import time, shutil, os, glob, ConfigParser, sys, getopt
try:
...
class sphy(pcrm.DynamicModel):
	def __init__(self, cfgfile):
		...
		self.GlacFLAG = self.config.getint('MODULES','GlacFLAG')
		self.SnowFLAG = self.config.getint('MODULES','SnowFLAG')
		self.RoutFLAG = self.config.getint('MODULES','RoutFLAG')
		self.ResFLAG = self.config.getint('MODULES','ResFLAG')
		self.DynVegFLAG = self.config.getint('MODULES','DynVegFLAG')
		self.GroundFLAG = self.config.getint('MODULES','GroundFLAG')
		...
		self.inpath = self.config.get('DIRS', 'inputdir')
		self.outpath = self.config.get('DIRS', 'outputdir')

		#-set the timing criteria
		sy = self.config.getint('TIMING', 'startyear')
		sm = self.config.getint('TIMING', 'startmonth')
		sd = self.config.getint('TIMING', 'startday')
		ey = self.config.getint('TIMING', 'endyear')
		em = self.config.getint('TIMING', 'endmonth')
		ed = self.config.getint('TIMING', 'endday')
		...
		self.mm_rep_FLAG = self.config.getint('REPORTING','mm_rep_FLAG')
			
		#-setting clone map
		clonemap = self.inpath + self.config.get('GENERAL','mask')
		pcr.setclone(clonemap)
		self.clone = pcr.readmap(clonemap)
		
		#-read general maps
		self.DEM = pcr.readmap(self.inpath + self.config.get('GENERAL','dem'))
		self.Slope = pcr.readmap(self.inpath + self.config.get('GENERAL','Slope'))
		self.Sub = pcr.readmap(self.inpath + self.config.get('GENERAL','Sub'))
		self.Locations = pcr.readmap(self.inpath + self.config.get('GENERAL','locations'))
		
		
		#-read soil maps
		#self.Soil = pcr.readmap(self.inpath + config.get('SOIL','Soil'))
		self.RootFieldMap = pcr.readmap(self.inpath + self.config.get('SOIL','RootFieldMap'))
		self.RootSatMap = pcr.readmap(self.inpath + self.config.get('SOIL','RootSatMap'))
		self.RootDryMap = pcr.readmap(self.inpath + self.config.get('SOIL','RootDryMap'))
		self.RootWiltMap = pcr.readmap(self.inpath + self.config.get('SOIL','RootWiltMap'))
		self.RootKsat = pcr.readmap(self.inpath + self.config.get('SOIL','RootKsat'))
		self.SubSatMap = pcr.readmap(self.inpath + self.config.get('SOIL','SubSatMap'))
		self.SubFieldMap = pcr.readmap(self.inpath + self.config.get('SOIL','SubFieldMap'))
		self.SubKsat = pcr.readmap(self.inpath + self.config.get('SOIL','SubKsat'))
		...
		for i in pars:
			try:
				setattr(self, i, pcr.readmap(self.inpath + self.config.get('SOILPARS',i)))
			except:
				setattr(self, i, self.config.getfloat('SOILPARS',i))
		if self.GroundFLAG == 0:   # if groundwater module is not used, read seepage and gwl_base
			self.SeepStatFLAG = self.config.getint('SOILPARS','SeepStatic')
			if self.SeepStatFLAG == 0: # set the seepage map series
				self.Seepmaps = self.inpath + self.config.get('SOILPARS', 'SeePage')
			else: #-set a static map or value for seepage
				try:
					self.SeePage = pcr.readmap(self.inpath + self.config.get('SOILPARS','SeePage'))
				except:
					self.SeePage = self.config.getfloat('SOILPARS','SeePage')
			try:
				self.GWL_base = pcr.readmap(self.inpath + self.config.get('SOILPARS','GWL_base'))
			except:
				self.GWL_base = self.config.getfloat('SOILPARS','GWL_base')
			...
			for i in pars:
				try:
					setattr(self, i, pcr.readmap(self.inpath + self.config.get('GROUNDW_PARS',i)))
				except:
					setattr(self, i, self.config.getfloat('GROUNDW_PARS',i))
		...
		if self.DynVegFLAG == 0:
			self.KcStatFLAG = self.config.getint('LANDUSE', 'KCstatic')
			if self.KcStatFLAG == 1:
				#-read land use map and kc table
				self.LandUse = pcr.readmap(self.inpath + self.config.get('LANDUSE','LandUse'))
				self.kc_table = self.inpath + self.config.get('LANDUSE','CropFac')
				self.Kc = pcr.lookupscalar(self.kc_table, self.LandUse)
			else:
				#-set the kc map series
				self.Kcmaps = self.inpath + self.config.get('LANDUSE', 'KC')
		#-Use the dynamic vegetation module
		else:
			#-set the ndvi map series to be read
			self.ndvi = self.inpath + self.config.get('DYNVEG', 'NDVI')
			#-read the vegetation parameters
			pars = ['NDVImax','NDVImin','NDVIbase','KCmax','KCmin','LAImax','FPARmax','FPARmin']
			for i in pars:
				try:
					setattr(self, i, pcr.readmap(self.inpath + self.config.get('DYNVEG', i)))
				except:
					setattr(self, i, self.config.getfloat('DYNVEG', i))
			
		#-read and set glacier maps and parameters if glacier module is used
		if self.GlacFLAG == 1:
			self.GlacFracCI = pcr.readmap(self.inpath + self.config.get('GLACIER','GlacFracCI'))
			self.GlacFracDB = pcr.readmap(self.inpath + self.config.get('GLACIER','GlacFracDB'))
			pars = ['DDFG','DDFDG','GlacF']
			for	i in pars:
				try:
					setattr(self, i, pcr.readmap(self.inpath + self.config.get('GLACIER',i)))
				except:
					setattr(self, i, self.config.getfloat('GLACIER',i))
		...
		if self.SnowFLAG == 1:
			...
			for	i in pars:
				try:
					setattr(self, i, pcr.readmap(self.inpath + self.config.get('SNOW',i)))
				except:
					setattr(self, i, self.config.getfloat('SNOW',i))
		
		#-read and set climate forcing and the calculation of etref
		self.Prec = self.inpath + self.config.get('CLIMATE','Prec')
		self.Tair = self.inpath + self.config.get('CLIMATE','Tair')
		self.ETREF_FLAG = self.config.getint('ETREF','ETREF_FLAG')
		#-determine the use of a given etref time-series or calculate etref using Hargreaves
		if self.ETREF_FLAG == 1:
			self.ETref = self.inpath + self.config.get('ETREF','ETref')
		else:
			self.Lat = pcr.readmap(self.inpath + self.config.get('ETREF','Lat'))
			self.Tmax = self.inpath + self.config.get('ETREF','Tmax')
			self.Tmin = self.inpath + self.config.get('ETREF','Tmin')
			self.Gsc = self.config.getfloat('ETREF', 'Gsc')
			import hargreaves
		...
		if self.RoutFLAG == 1 or self.ResFLAG == 1:
			self.FlowDir = pcr.readmap(self.inpath + self.config.get('ROUTING','flowdir'))
			try:
				self.kx = pcr.readmap(self.inpath + self.config.get('ROUTING','kx'))
			except:
				self.kx = self.config.getfloat('ROUTING','kx')
		
		#-read and set reservoir maps and parameters
		if self.ResFLAG == 1:
			self.LakeID = pcr.readmap(self.inpath + self.config.get('RESERVOIR','lakeid'))
			try:
				self.UpdateLakeLevel = pcr.readmap(self.inpath + self.config.get('RESERVOIR','updatelakelevel'))
				self.LLevel = self.config.get('RESERVOIR','LakeFile')
			except:
				pass
			#-qh-function maps and parameters
			self.qh_function = pcr.readmap(self.inpath + self.config.get('RESERVOIR','qh_function'))
			self.qh_exp_a = pcr.readmap(self.inpath + self.config.get('RESERVOIR','qh_exp_a'))
			self.qh_exp_b = pcr.readmap(self.inpath + self.config.get('RESERVOIR','qh_exp_b'))
			self.qh_pol_b = pcr.readmap(self.inpath + self.config.get('RESERVOIR','qh_pol_b'))
			self.qh_pol_a1 = pcr.readmap(self.inpath + self.config.get('RESERVOIR','qh_pol_a1'))
			self.qh_pol_a2 = pcr.readmap(self.inpath + self.config.get('RESERVOIR','qh_pol_a2'))
			self.qh_pol_a3 = pcr.readmap(self.inpath + self.config.get('RESERVOIR','qh_pol_a3'))
			#-hs-function maps and parameters
			self.hs_function = pcr.readmap(self.inpath + self.config.get('RESERVOIR','hs_function'))
			self.hs_exp_a = pcr.readmap(self.inpath + self.config.get('RESERVOIR','hs_exp_a'))
			self.hs_exp_b = pcr.readmap(self.inpath + self.config.get('RESERVOIR','hs_exp_b'))
			self.hs_pol_b = pcr.readmap(self.inpath + self.config.get('RESERVOIR','hs_pol_b'))
			self.hs_pol_a1 = pcr.readmap(self.inpath + self.config.get('RESERVOIR','hs_pol_a1'))
			self.hs_pol_a2 = pcr.readmap(self.inpath + self.config.get('RESERVOIR','hs_pol_a2'))
			self.hs_pol_a3 = pcr.readmap(self.inpath + self.config.get('RESERVOIR','hs_pol_a3'))
			#-sh-function maps and parameters
			self.sh_function = pcr.readmap(self.inpath + self.config.get('RESERVOIR','sh_function'))
			self.sh_exp_a = pcr.readmap(self.inpath + self.config.get('RESERVOIR','sh_exp_a'))
			self.sh_exp_b = pcr.readmap(self.inpath + self.config.get('RESERVOIR','sh_exp_b'))
			self.sh_pol_b = pcr.readmap(self.inpath + self.config.get('RESERVOIR','sh_pol_b'))
			self.sh_pol_a1 = pcr.readmap(self.inpath + self.config.get('RESERVOIR','sh_pol_a1'))
			self.sh_pol_a2 = pcr.readmap(self.inpath + self.config.get('RESERVOIR','sh_pol_a2'))
			self.sh_pol_a3 = pcr.readmap(self.inpath + self.config.get('RESERVOIR','sh_pol_a3'))	
	...
	def initial(self):
		...
		if not self.config.get('SOIL_INIT','RootWater'):
			self.RootWater = self.RootField
		else:
			try:
				self.RootWater = self.config.getfloat('SOIL_INIT','RootWater')
			except:
				self.RootWater = pcr.readmap(self.inpath + self.config.get('SOIL_INIT','RootWater'))
		#-initial water content in subsoil
		if not self.config.get('SOIL_INIT','SubWater'):
			self.SubWater = self.SubField
		else:
			try:
				self.SubWater = self.config.getfloat('SOIL_INIT','SubWater')
			except:
				self.SubWater = pcr.readmap(self.inpath + self.config.get('SOIL_INIT','SubWater'))
		#-initial water storage in rootzone + subsoil
		self.SoilWater = self.RootWater + self.SubWater
		#-initial capillary rise
		try:
			self.CapRise = self.config.getfloat('SOIL_INIT','CapRise')
		except:
			self.CapRise = pcr.readmap(self.inpath + self.config.get('SOIL_INIT','CapRise'))		
		#-initial drainage from rootzone
		try:
			self.RootDrain = self.config.getfloat('SOIL_INIT','RootDrain')
		except:
			self.RootDrain = pcr.readmap(self.inpath + self.config.get('SOIL_INIT','RootDrain'))		
		...
		if self.GroundFLAG == 1:
			...
			try:
				self.GwRecharge = self.config.getfloat('GROUNDW_INIT','GwRecharge')
			except:
				self.GwRecharge = pcr.readmap(self.inpath + self.config.get('GROUNDW_INIT','GwRecharge'))
			#-initial baseflow
			try:
				self.BaseR = self.config.getfloat('GROUNDW_INIT','BaseR')
			except:
				self.BaseR = pcr.readmap(self.inpath + self.config.get('GROUNDW_INIT','BaseR'))
			#-initial groundwater storage
			try:
				self.Gw = self.config.getfloat('GROUNDW_INIT','Gw')
			except:
				self.Gw = pcr.readmap(self.inpath + self.config.get('GROUNDW_INIT','Gw'))
			#-initial groundwater level
			try:
				self.H_gw = self.config.getfloat('GROUNDW_INIT','H_gw')
			except:
				self.H_gw = pcr.readmap(self.inpath + self.config.get('GROUNDW_INIT','H_gw'))
			self.H_gw = pcr.max((self.RootDepthFlat + self.SubDepthFlat + self.GwDepth)/1000 - self.H_gw, 0)	
		else:
			#-initial drainage from subsoil
			try:
				self.SubDrain = self.config.getfloat('SOIL_INIT','SubDrain')
			except:
				self.SubDrain = pcr.readmap(self.inpath + self.config.get('SOIL_INIT','SubDrain'))
		...
		if self.SnowFLAG == 1:
			try:
				self.SnowStore = self.config.getfloat('SNOW_INIT','SnowIni')
			except:
				self.SnowStore = pcr.readmap(self.inpath + self.config.get('SNOW_INIT','SnowIni'))			
			#-initial water stored in snowpack
			try:
				self.SnowWatStore = self.config.getfloat('SNOW_INIT','SnowWatStore')
			except:
				self.SnowWatStore = pcr.readmap(self.inpath + self.config.get('SNOW_INIT','SnowWatStore'))
			self.TotalSnowStore = self.SnowStore + self.SnowWatStore	

		#-initial glacier properties
		if self.GlacFLAG == 1:
			try:
				self.GlacFrac = self.config.getfloat('GLACIER_INIT','GlacFrac')
			except:
				self.GlacFrac = pcr.readmap(self.inpath + self.config.get('GLACIER_INIT','GlacFrac'))
			
		#-initial routed total runoff and of individual components
		if self.RoutFLAG == 1 or self.ResFLAG == 1:
			#-initial routed total runoff
			try:
				self.QRAold = self.config.getfloat('ROUT_INIT','QRA_init')
			except:
				self.QRAold = pcr.readmap(self.inpath + self.config.get('ROUT_INIT','QRA_init'))
			#-initial routed runoff	for the individual components
			pars = ['RainRA','BaseRA','SnowRA','GlacRA']
			for i in pars:
				try:
					setattr(self, i + 'old', pcr.readmap(self.inpath + self.config.get('ROUT_INIT', i + '_init')))
					setattr(self, i + '_FLAG', 1)
				except:
					try:
						setattr(self, i + 'old', self.config.getfloat('ROUT_INIT', i + '_init'))
						setattr(self, i + '_FLAG', 1)
					except:
		...
		if self.ResFLAG == 1:
			...
			try:
				self.StorAct = pcr.readmap(self.inpath + self.config.get('RESER_INIT', 'StorInit'))
			except:
				self.StorAct = self.config.getfloat('RESER_INIT', 'StorInit')
			#-initial storage in reservoirs of individual components
			pars = ['RainRA','BaseRA','SnowRA','GlacRA']
			for i in pars:
				if eval('self.' + i + '_FLAG') == 1:
					try:
						setattr(self, i + 'stor', pcr.readmap(self.inpath + self.config.get('RESER_INIT', i + '_istor')))
					except:
						setattr(self, i + 'stor', self.config.getfloat('RESER_INIT', i + '_istor'))
		...
		for i in pars:
			mapoutops = self.config.get('REPORTING', i+'_mapoutput')
			TSoutops = self.config.get('REPORTING', i+'_TSoutput')
			if mapoutops == 'NONE' and TSoutops == 'NONE':
				print i + ' will NOT be reported'
			else:
				print i + ' will be reported'
				fname = self.config.get('REPORTING', i+'_fname')
				setattr(self, i+'_fname', fname)
				try:
					setattr(self, i, pcr.readmap(self.inpath + self.config.get('INITTOT', i)))
				except:
					try:
						setattr(self, i, self.config.getfloat('INITTOT', i)) 
					except:
						setattr(self, i, 0.)
				if mapoutops != 'NONE':
